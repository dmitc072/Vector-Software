  ...
  shapes.forEach((shapeData) => {
    if (event.target.id === shapeData.id) {
      ...
      if (shapeData.type === "circle") {
        radiusOrWidthLabel.textContent =
          "Enter the new radius for this circle:";
        changeWidth.value = "";
        changeHeight.value = "";
        inputWidth.setAttribute("disabled", true);
        inputHeight.setAttribute("disabled", true);
      ...
      changeStrokeSize.value =
        previousShapeElement.getAttribute("stroke-width");
    }
  });
});
...
arrangementDropdown.addEventListener("change", function () {
  ...
  shapes.forEach((shapeData) => {
    ...
    let firstShapeChangeIndex = shapes.findIndex(
      (shape) => shape.id === firstShapeChange.value
    );
    ...
    let secondShapeChangeIndex = shapes.findIndex(
      (shape) => shape.id === secondShapeChange.value
    );
    let removeShape;

    switch (selectedValue) {
      case "front":
        firstShapeChangeIndex = shapes.findIndex(
          (shape) => shape.id === firstShapeChange.value
        ); //ex rect1[0]
      ...
      case "before":
        ...
        secondShapeChangeIndex = shapes.findIndex(
          (shape) => shape.id === secondShapeChange.value
        ); //ex rect3[2]
      ...
      case "after":
        ...
        console.log(
          "firstShapeChange.value is " +
            firstShapeChange.value +
            " and the index is " +
            firstShapeChangeIndex
        );
        console.log(
          "secondShapeChange.value is " +
            secondShapeChange.value +
            " and the index is " +
            secondShapeChangeIndex
        );
        ...
        console.log(
          "the new firstShapeChange.value is " +
            firstShapeChange.value +
            " and the index is " +
            firstShapeChangeIndex
        );
        console.log(
          "the new secondShapeChange.value is " +
            secondShapeChange.value +
            " and the index is " +
            secondShapeChangeIndex
        );
...
createChangeButton.addEventListener("click", (event) => {
  shapes.forEach((shapeData) => {
    ...
    if (currentSelectedShape.id === shapeData.id) {
      const previousShapeElement = document.getElementById(
...
function moveObjects(object, event) {
  ...
  const initialPositionXForPrompt =
    event.clientX - object.offsetParent.offsetLeft; //needs to be outside the if function about prompt, or it move quickly
  const initialPositionYForPrompt =
    event.clientY - object.offsetParent.offsetTop; //needs to be outside the if function about prompt, or it move quickly

  if (
    event.target.id !== "selectionIcon" &&
    event.target.nodeName === object.nodeName
  ) {
    isObjectMoving = true;
    const onMouseMove = (mousemoveEvent) => {
      ...
      if (isObjectMoving && object.offsetParent.className === "prompt") {
        //because of customprompt has to forms, I was able to call the parent this way
    ...
    if (event.target.id === shapeData.id) {
      ...
      const onMouseMove = (mousemoveEvent) => {
        if (isAdjustingShape && event.target.id !== "design") {
          ...
          if (event.target.tagName === "line") {
            if (
              event.clientX >= initialShapeX1 - 20 &&
              event.clientX <= initialShapeX1 + 20
            ) {
            ...
            if (
              event.clientX >= initialShapeX2 - 20 &&
              event.clientX <= initialShapeX2 + 20
            ) {
...
function renderHistory() {
  //located in mouseup function in each shape file
